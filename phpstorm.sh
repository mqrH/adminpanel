...
# Try (in order): $PHPSTORM_JDK, .../phpstorm.jdk, .../jbr, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
JRE=""

# shellcheck disable=SC2154
if [ -n "$PHPSTORM_JDK" ] && [ -x "$PHPSTORM_JDK/bin/java" ]; then
  JRE="$PHPSTORM_JDK"
fi

if [ -z "$JRE" ] && [ -s "${CONFIG_HOME}/JetBrains/PhpStorm2024.1/phpstorm.jdk" ]; then
  USER_JRE=$(cat "${CONFIG_HOME}/JetBrains/PhpStorm2024.1/phpstorm.jdk")
...
if [ -z "$JAVA_BIN" ] || [ ! -x "$JAVA_BIN" ]; then
  message "No JRE found. Please make sure \$PHPSTORM_JDK, \$JDK_HOME, or \$JAVA_HOME point to valid JRE installation."
  exit 1
fi
...
# shellcheck disable=SC2154
if [ -n "$PHPSTORM_VM_OPTIONS" ] && [ -r "$PHPSTORM_VM_OPTIONS" ]; then
  # 1. $<IDE_NAME>_VM_OPTIONS
  VM_OPTIONS_FILE="$PHPSTORM_VM_OPTIONS"
else
  # 2. <IDE_HOME>/bin/[<os>/]<bin_name>.vmoptions ...
  if [ -r "${IDE_BIN_HOME}/phpstorm64.vmoptions" ]; then
    VM_OPTIONS_FILE="${IDE_BIN_HOME}/phpstorm64.vmoptions"
  else
    test "${OS_TYPE}" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
    if [ -r "${IDE_BIN_HOME}/${OS_SPECIFIC}/phpstorm64.vmoptions" ]; then
      VM_OPTIONS_FILE="${IDE_BIN_HOME}/${OS_SPECIFIC}/phpstorm64.vmoptions"
    fi
  fi
  ...
  elif [ -r "${CONFIG_HOME}/JetBrains/PhpStorm2024.1/phpstorm64.vmoptions" ]; then
    USER_VM_OPTIONS_FILE="${CONFIG_HOME}/JetBrains/PhpStorm2024.1/phpstorm64.vmoptions"
  fi
fi
